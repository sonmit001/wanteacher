<%@page import="kr.or.css.dto.ContentList_DTO"%>
<%@page import="kr.or.css.action.ContentList"%>
...
<head>
	...
	<%  
        ...
        int totalboardCount = (Integer)request.getAttribute("totalContentCount");
        List<ContentList_DTO> list = (List)request.getAttribute("list");
        int firstpage = ((cpage-1)/5)*5;
        int endck = 0;
        if((firstpage + 5) >= pagecount){
        	endck = 1;
        }
        ...
        System.out.println("endck : " + endck);
	%>
			...
			$.ajax({
				...
				success: function(data){
					...
					$('.delete').on('click', function(){
						...
						location.href = "deleteContent.db?contentnum=" + $(this).attr('seq') + "&cp=" + <%= cpage%> + "&ps=" + <%=pagesize%>;
					});
					...
					$('.modify').on('click', function(){
						...
						location.href = "contentedit.db?contentnum=" + $(this).attr('seq') + "&cp=" + <%= cpage%> + "&ps=" + <%=pagesize%>;
		...
		<div class="col-sm-12">
			<div class="col-sm-3 left">
				<div class="input-group">
					<input type="text" class="form-control" placeholder="번호 검색"
						id="contentnum" style="padding-bottom: 7px;">
			...
			<div class="col-sm-2 right">
				<form name="list" action="contentList.db">
		...
		<div id="tablediv">
			<table class="table table-bordered">
		        ...
		        <c:forEach var="content" items="<%=list%>">
					<c:set var="contentnum" value="${content.contentnum}" />
					<c:set var="title" value="${content.title}" />
					<c:set var="themename" value="${content.themename}" />
					<c:set var="regionname" value="${content.regionname}" />
					<tr>
						<td>${contentnum}</td>
						<td>${title}</td>
						<td>${themename}</td>
						<td>${regionname}</td>
						<td><button class="btn btn-success btn-xs text modify" style="padding:2px 5px;" type="button" seq="${contentnum}" >수정하기</button></td>
						<td><button class="btn btn-danger btn-xs text delete" style="padding:2px 5px;" type="button" seq="${contentnum}" >삭제하기</button></td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="6">
						<ul class="pagination">
							<!--이전 링크 --> 
							<c:if test="${cpage>1}">
								<li><a href="contentList.db?cp=${cpage-1}&ps=${pagesize}">이전</a></li>
								<!--페이지 리스트 구현  -->
							</c:if>
							<c:choose>
								<c:when test="${endck == 0}">
									<c:forEach var="i" begin="${firstpage+1}" end="${firstpage+5}" step="1">
										<c:choose>
											...
											<c:otherwise>
												<li><a href="contentList.db?cp=${i}&ps=${pagesize}">[${i}]</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:when>
							...
							<c:if test="${cpage<pagecount}">
								<li><a href="contentList.db?cp=${cpage+1}&ps=${pagesize}">다음</a></li>
							</c:if>
